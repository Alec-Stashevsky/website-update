...
For each node we have added a feature vector which holds some information about those user-profiles such as how long they have been on the platform $x_1$, when they last logged on $x_2$, there geographic region $x_3$, and so on. 

<!-- Might delete the edge features, not necessary -->
<!-- We can also add edge features which can come from a different feature space with different dimensionality. These can hold information regarding the friendship (which must occur between two nodes) such as how long the users have been friends $y_1$ and how often the message each other $y_2$. -->

<!-- ![](~/Git/website/content/post/topology-vs-geometry/basic_graph-1-node--edge-features.svg) -->
<!-- End edge-features -->

Assigning feature vectors to the graph overlays a geometry upon it. We can no longer move, bend, or switch nodes on the graph because each node now exists as a certain point in our feature-vector space. 
