...
For machines to learn on data, be it graph-structured or not, we must represent the data in a way computers can digest. That is to say, we must encode the data into some *n*-dimensional Euclidean vector space where individual data points are vectors within that space.
...
Relational data is the most widely used type of data across industry, yet it is not often thought about in terms of its inherent graph structure.^[cite] A simple database representation of our small social network below depicts the inherent relational structure which can be explicitly embedded as the edges of a graph.
...
Topological structure need not conform to the Euclidean feature-space. Graphs force us to understand this through their non-euclidean connectivity. However, a different kind of topology has been lurking alongside us all along. 

When we think about the data most machine learning operates on such as tables, text, or images, we don't usually speak about topology. That is because these data types all enforce a strict *regular topology* that doesn't give us much signal to learn on. Images, for example, are represented as a regular grid of pixels values which can also be transformed into a graph like below.^[Figure from [Yalda Shankar's](https://yaldashankar.org) beautifully crafted visual-forward Graph ML overview: [The GNN Booklet](https://yaldashankar.org/demo/GNN-1.pdf).]

![](image_as_graph.png)

The structure of the pixel-grid will not change from image to image. That means the topology of the data can't help us predict what is in one image vs. another. All of that signal is held in the 2-d Euclidean pixel-space rather than the topology of the pixel-space. The same case can be made for text data in a 1-d space, or any *n*-dimensional Euclidean vector space (like that of our Condor GPS Telemetry) because there is always regular grid structure.

Machine learning canon evolved using Euclidean data and enforced this regular topology implicitly as an artifact. As a result canonical ML has been indexed to specific data with strict limitations on input structure.
<!-- (say something about 2-d image vs 3d world its trying to model, only 1-d text sentatnces ) -->
<!-- , putting strict limitations how inputs must be represented on the resolution of images or length of text models can ingest.^[Typically CNNs will ingest images with dimensions 512x512 pixels. Transformers typically have a limit of 512 tokens.]  -->
[Geometric Deep Learning](https://flawnsontong.medium.com/what-is-geometric-deep-learning-b2adb662d91d) is carving a different paradigm in which models are designed to understand and exploit the topological and geometric properties of data. Models designed through this paradigm can adapt to different and more complex topology like graphs, manifolds, and meshes (link) with dynamically-sized inputs.

<br>

## To Blog Is To Forget

Once brought into the fold, it can be hard to stop seeing graphs everywhere you look and especially hard to not send your CV and NLP colleagues blog posts and how [CNNs](https://medium.com/@BorisAKnyazev/tutorial-on-graph-neural-networks-for-computer-vision-and-beyond-part-1-3d9fada3b80d) and [Transformers](https://thegradient.pub/transformers-are-graph-neural-networks) are just Graph Neural Networks. Pretty much everything is a graph or can be molded into one, but doing so isn't always useful. Enforcing a regular topology help not distract the model when there is important information in the topology of the data. For example, take the image graphed from above, there is no interesting local/ neighborhood information within that structure that can help learn about how that image is different from another structured collection of pixels. 

-- in document AI, there has been recent application of Graphs, which attempt to use them to uncover a *semantic* topology. 
...
- this is not to say topological structure cannot also confirm to Euclidean notions. 
- Talk about uncovering a *semantic topology*
- talk about document AI, 
- close


An interconnected world matter, and graphs provide the scafolding to build such connectivity. (cite NeurIPS 2022 workshop)
...
-- link to drug discovery https://doi.org/10.1016/j.cell.2020.01.021
