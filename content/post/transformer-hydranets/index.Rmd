...
Welcome to this series of blog posts that is not about ensemble learning. Instead, this series focuses on a less well-known modeling paradigm that seeks to exploit, rather than ignore, collaboration. This is the paradigm of *multi-task learning* (MTL); a field concerned with training models to perform more than one task at the same time. I find [Sebasatian Ruder's](https://www.ruder.io/about/) explanation to be the best:^[https://www.ruder.io/multi-task/#introduction.]
...
Both tasks can be seen as a type of sequence classification. Let's start by stealing some code like all good engineers do! I'll use the source code for `RobertaForSequenceClassification` and we will build out the first head to perform binary sequence classification and the second head perform multi-class classification.^[We can consider the class labels as $[harm, not \ harm]$ for head 1 and $[spam, hate, promotion, not \ harm]$ for head 2.] I've renamed a few things and added separate configs as the output dimensions are different between head 1 and 2:^[Specically the `num_labels` attribute of the config needs to be set to $1$ for head 1 and $4$ for head 2 based on our class labels defined in [[5]](http://localhost:4321/post/transformer-hydranets-and-multi-task-learning-with-hugging-face-and-pytorch-ensembling-vs.-eclat/#fn5).]
