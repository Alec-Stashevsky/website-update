...
### Disadvantages
MTL systems can be prone to imbalances between tasks and competition for influence over the model backbone parameters leading to catastrophic forgetting, or bottlenecks in the learning process requiring sophisticated techniques in model design, loss function balancing, and training regimen. The flip side of entwinement is the lack of modularity---a key property of ensembles. Adding new tasks or models is much harder with hydranets as it becomes entangled with the rest of the system performance. It is possible to isolate taska and achieve modularity with MTL by careful freezing of weights, but it is nonetheless more challenging.
...
Building a hydranet is one way to force yourself to recognize the beauty of PyTorch [Autograd](https://pytorch.org/tutorials/beginner/blitz/autograd_tutorial.html). The essential feature of hydranet architectures is that your model's forward pass returns multiple things.
